...
- 새 하네스의 문서 루트는 `.ai-docs/` 하나뿐이다.
    ...
    - 이전 `.docs/`만 있으면 일반 세팅을 진행하지 않고 이관 계획과 별도 승인을 거친다.
    - 서명된 권한 정책이 있으면 `admin`이 `project-write-access`의 `migrate-root-plan`과 `migrate-root`로 정책·Git 훅·AI 가드 경로를 함께 이관한다.
    - `.docs/`와 `.ai-docs/`가 함께 있으면 자동 병합하지 않고 충돌로 중단한다.
